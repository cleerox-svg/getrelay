...
> **Relay status note.** The app's Discover tab placeholder has been replaced by the **Games** hub (tab label "Games", routed at `/games`; `/discover` redirects there): a grid of mini games — Fog (a steamed-up window the player wipes to reveal a mystery image), Guess the Tune, Golf and Baseball — with a server-backed leaderboard (`POST /game/score`, `GET /game/leaderboard`). The Channels-era Discover material in this chapter remains historical reference for a future Channels build.
