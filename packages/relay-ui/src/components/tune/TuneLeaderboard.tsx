...
// Weekly / all-time "Guess the Tune" leaderboard. Clone of
// FogLeaderboard that asks the worker for the `tune` board. Rows come
// pre-ranked; the caller's own row arrives flagged `mine`.
export function TuneLeaderboard({ refreshKey = 0 }: Props) {
  ...
  useEffect(() => {
    ...
    setError(false);
    api
      .getGameLeaderboard(period, 'tune')
      .then((r) => {
        ...
        >
          {entries.map((e, i) => (
            ...
            >
              ...
              <span className="text-xs tabular-nums w-14 text-right shrink-0" style={{ color: 'var(--text-dim)' }}>
