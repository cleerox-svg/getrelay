...
| `0013_golf_economy.sql` | Golf economy + progression. Create `user_wallet` (one row per user, coin balance / soft currency), `currency_ledger` (append-only audit of every balance change + idempotency guard — partial `UNIQUE(user_id, reason, ref) WHERE ref IS NOT NULL` so an award credits once), `user_cosmetics` (owned inventory, `UNIQUE(user_id, cosmetic_id)`; catalog is code-defined, no catalog table), `user_equipped` (equipped cosmetic per slot, `PK(user_id, slot)`) and `season_progress` (per-user per-season `xp` + `claimed` tiers, `PK(user_id, season_id)`; season/tier layout derived in code). Idempotent. | Same as above, with `file: 0013_golf_economy.sql` |
| `0014_baseball_cards.sql` | Baseball progression. Create `player_cards` (per-user card ownership with a `level` and a duplicate-`copies` count, `UNIQUE(user_id, card_id)`; catalog is code-defined, no catalog table), `user_lineup` (equipped card per slot, `PK(user_id, slot)`, slots batter/pitcher/bat/glove/coach), `ladder_state` (per-user ranked ladder: trophies + resolved rank tier, one row per user) and `pack_grants` (pack awards + idempotency guard — partial `UNIQUE(user_id, reason, ref) WHERE ref IS NOT NULL`, structurally identical to `currency_ledger`; the `reason` column is load-bearing, since one source event can grant two pack types under one `ref`). Only these four: new cosmetic slots and new game ids need NO migration (`user_equipped.slot` and `game_scores.game` are free-form TEXT); a card's level+copies and a trophy ladder are what the existing tables cannot carry. Idempotent. | Same as above, with `file: 0014_baseball_cards.sql` |
